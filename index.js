...
const start = () => {
    ...
    bot.on('message', async msg => {
        ...
        try {
            if (text === '/start') {
                ...
                return bot.sendMessage(chatId, `Welcome to the bot created by @ohanovdmytro`);
            }
            if(text === '/info') {
    ...
    bot.on('callback_query', async msg => {
        ...
        if (data == chats[chatId]) {
            await bot.sendMessage(chatId, `Great! I have ordered number ${chats[chatId]}!`, againOptions);
        } else {
